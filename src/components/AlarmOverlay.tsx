      ...
      {ringing && (
        ...
        >
          <span className="sr-only" aria-live="assertive">
            Alarm ringing. It is{" "}
            {alarmTime ? (use24h ? alarmTime : format12(alarmTime)) : ""}. Press
            stop to dismiss.
          </span>
          {/* Pulsing red beams */}
          <div className="absolute inset-0 overflow-hidden">
            ...
            {[...Array(12)].map((_, i) => (
              <div
                key={i}
                className="absolute left-1/2 top-1/2 w-[140vmax] h-24 -translate-x-1/2 -translate-y-1/2 origin-center animate-[alarmBeam_3s_linear_infinite]"
                style={{
                  background:
                    "linear-gradient(to right, transparent, rgba(255,0,0,0.75), transparent)",
          ...
          >
            <div className="text-center space-y-2 select-none">
              ...
              >
                It's{" "}
                {alarmTime ? (use24h ? alarmTime : format12(alarmTime)) : ""}.
                Tap stop to dismiss.
              </motion.p>
            </div>
            <button
              onClick={stop}
              className="px-6 py-3 rounded-lg bg-red-600 hover:bg-red-500 active:scale-95 transition text-white font-medium shadow-lg shadow-red-800/40"
            >
              Stop Alarm
            </button>
          </motion.div>
            ...
            @keyframes alarmBeam {
              ...
              100% {
                transform: translate(-50%, -50%)
                  rotate(calc(var(--r, 0deg) + 360deg)) translateY(0);
              }
            }
